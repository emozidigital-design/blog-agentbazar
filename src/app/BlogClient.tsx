...
function BlogContent({ initialPosts, initialTotal }: BlogClientProps) {
...
export default function BlogPage({ initialPosts, initialTotal }: BlogClientProps) {
  return (
    <Suspense fallback={<div style={{ padding: 40, textAlign: 'center', color: '#aaa' }}>Loading...</div>}>
      <BlogContent initialPosts={initialPosts} initialTotal={initialTotal} />
    </Suspense>
  )
}
